...
namespace Kassa
{
    public partial class Form_Start : MaterialForm
    {
        ...
        private ToolTip notificationToolTip; // Всплывающее уведомление при наведении
        ...
        public Form_Start()
        {
            ...
            var errorSnackbar = new ErrorSnackbar();
        }
        private void InitializeNotification()
        {
            ...
            notificationToolTip.SetToolTip(buttonCopy15, "Двойной клик чтобы скопировать название ОФД без аббревиатур");
        }
        ...
        private void Director_org_Leave(object sender, EventArgs e)
        {
            ...
            if (directorOrganization_array.Length < 2 && TextBox_Director_org.Text.Length > 0)
            {
        ...
        private void KPP_organization_Leave(object sender, EventArgs e) // Проверка КПП Организации
        {
            ...
            else if (TextBox_KPP_organization.Text.Length == 9)
            {
        ...
        private void PlaceSale_Enter(object sender, EventArgs e) // Место расчетов
        {
            if (TextBox_PlaceSale.Text == "")
            {
        ...
        private async void PlaceSale_Leave(object sender, EventArgs e) // Место расчетов
        {
            await Delay.ExecuteWithDelay(100);
            buttonInsertValue.Visible = false;
        }
        ...
        private void butSave_Click(object sender, EventArgs e) // кнопка Сохранить
        {
            DataKKTManager();
            Save s = new Save();
            s.SaveData(dataKKT, settings, this);
                        ...
                        for (int i = 0; i < (dataFileLine.Length - 1); i = i + 2)
                        {
                            ...
                            switch (key)
                            {
                                ...
                                case "Модель ККТ":
                                    ...
                                    break;
                                case "Номер автомата":
                                    TextBox_Number_automatic.Text = value;
                                    break;
                                case "Номер ФН":
        ...
        private void butReaddata_Click(object sender, EventArgs e) //кнопка Считать данные
        {
            ...
            label_vers_FFD.Text = kktParameters.VersionFFD;
        ...
        private void buttonAkt_Click(object sender, EventArgs e) //кнопка Акт ввода в эксплуатацию
        {
            ...
            if (DanaT_FD[2] == '.' && DanaT_FD[5] == '.' && DanaT_FD[10] == ' ' && DanaT_FD[13] == ':')
            {
                ...
                if (TextBox_ID_client.Text == "")
                {
                    newakt = new CreateAkt("Akt_small.docx");
                }
                ...
                {
                    new MaterialSnackBar("Акт ввода сформирован, сохранен и отправлен на печать").Show(this);
                }
                else
                {
                    new MaterialSnackBar("Акт ввода сформирован и сохранен").Show(this);
                }
            }
            else
            {
        ...
        private void buttonAkt2_Click(object sender, EventArgs e) // Кнопка Акт ввода на второй странице
        {
            ...
            {
                file = new CreateAkt("Akt_small.docx");
            }
            ...
            string dataTime = TextBox_Date2.Text;
            string Date_FD = dataTime.Substring(0, 10);
            string T_FD = dataTime.Substring(Math.Max(0, dataTime.Length - 5));
        ...
        private void butSaveAKT_Click(object sender, EventArgs e) // Кнопка Сохранить на второй странцие
        {
            ...
            Save s = new Save();
            s.SaveData(dataKKTAkt, settings, this);
        ...
        private void TabControl_Selected(object sender, TabControlEventArgs e) // событие автозаполнения textBox настроек при активации вкладки
        {
            ...
            textBox_AdrMU_Region.Text = settings.AdrMU_Region;
            textBox_AdrMU_MunRay_Code.Text = settings.AdrMU_MunRay_Code;
            textBox_AdrMU_MunRay_Name.Text = settings.AdrMU_MunRay_Name;
            textBox_AdrMU_NasPunkt_type.Text = settings.AdrMU_NasPunkt_type;
            textBox_AdrMU_NasPunkt_Name.Text = settings.AdrMU_NasPunkt_Name;
            textBox_AdrMU_Street_type.Text = settings.AdrMU_Street_type;
            textBox_AdrMU_Street_name.Text = settings.AdrMU_Street_name;
            textBox_AdrMU_Building_type.Text = settings.AdrMU_Building_type;
            textBox_AdrMU_Building_number.Text = settings.AdrMU_Building_number;
        ...
        private void ButtonSave4_Click(object sender, EventArgs e) // Кнопка сохранение
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                success &= repo.UpdateParameter("id_fias", textBox_AdrMU_ID.Text);
                success &= repo.UpdateParameter("index", textBox_AdrMU_Index.Text);
                success &= repo.UpdateParameter("region", textBox_AdrMU_Region.Text);
                success &= repo.UpdateParameter("mr_code", textBox_AdrMU_MunRay_Code.Text);
                success &= repo.UpdateParameter("mr_name", textBox_AdrMU_MunRay_Name.Text);
                success &= repo.UpdateParameter("np_type", textBox_AdrMU_NasPunkt_type.Text);
                success &= repo.UpdateParameter("np_name", textBox_AdrMU_NasPunkt_Name.Text);
                success &= repo.UpdateParameter("street_type", textBox_AdrMU_Street_type.Text);
                success &= repo.UpdateParameter("street_name", textBox_AdrMU_Street_name.Text);
                success &= repo.UpdateParameter("building_type", textBox_AdrMU_Building_type.Text);
                success &= repo.UpdateParameter("building_number", textBox_AdrMU_Building_number.Text);
        ...
        private void buttonCopyModel_KKT_Click(object sender, EventArgs e)
        {
            if (TextBox_Model_KKT.Text != "")
            {
                ...
                new MaterialSnackBar("Скопировано").Show(this);
            }
            else
            {
                errorSnackbar.ShowErrorSnackbar(this, "Пустое поле");
            }
        ...
        private void buttonCopyName_OFD1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                new MaterialSnackBar($"Скопировано: {ComboBox_Name_OFD1.Text}").Show(this);
            }
        ...
        // Тестовая кнопка для проверки файла регистрации
